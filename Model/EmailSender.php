...
class EmailSender implements EmailSenderInterface
{
    ...
    private function sendDeveloperEmail(array $errorData, array $emails): bool
    {
        try {
            ...
            $transport = $this->buildDeveloperTransport($errorData, $emails);
    ...
    private function sendClientEmail(array $errorData, array $emails): bool
    {
        try {
            ...
            $transport = $this->buildClientTransport($clientData, $emails);
            $transport->sendMessage();
    ...
    /**
     * Build transport for developer email
     *
     * @param array<string, mixed> $errorData
     * @param array<int, string> $emails
     * @return \Magento\Framework\Mail\TransportInterface
     */
    private function buildDeveloperTransport(array $errorData, array $emails): \Magento\Framework\Mail\TransportInterface
    {
    ...
    /**
     * Build transport for client email
     *
     * @param array<string, mixed> $clientData
     * @param array<int, string> $emails
     * @return \Magento\Framework\Mail\TransportInterface
     */
    private function buildClientTransport(array $clientData, array $emails): \Magento\Framework\Mail\TransportInterface
    {
        return $this->transportBuilder
    ...
    /**
     * Build HTML email body for developers using Magento template rendering
     *
     * @param array<string, mixed> $errorData
     * @return string
     */
    ...
    {
        try {
            // Try to render using Magento template system
            $transport = $this->buildDeveloperTransport($errorData, ['dummy@example.com']);
            return $transport->getMessage()->getBody();
        } catch (\Throwable $e) {
            // If template rendering fails, return a simple fallback
            $this->logger->warning('Failed to render developer email template, using simple fallback', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);

            return $this->buildSimpleFallbackEmail($errorData, 'developer');
        }
    }

    /**
     * Build HTML email body for clients using Magento template rendering
     *
     * @param array<string, mixed> $errorData
     * @return string
     */
    private function buildClientEmailBody(array $errorData): string
    {
        try {
            ...
            // Try to render using Magento template system
            $transport = $this->buildClientTransport($clientData, ['dummy@example.com']);
            return $transport->getMessage()->getBody();
        } catch (\Throwable $e) {
            // If template rendering fails, return a simple fallback
            $this->logger->warning('Failed to render client email template, using simple fallback', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);

            return $this->buildSimpleFallbackEmail($errorData, 'client');
        }
    }

    /**
     * Build simple fallback email when template rendering fails
     *
     * @param array<string, mixed> $errorData
     * @param string $type Either 'developer' or 'client'
     * @return string
     */
    private function buildSimpleFallbackEmail(array $errorData, string $type): string
    {
        ...
        if ($type === 'developer') {
            ...
            $html .= '<div style="background-color: rgba(255,255,255,0.2); border-radius: 50%; width: 80px; height: 80px; margin: 0 auto 20px; display: flex; align-items: center; justify-content: center;"><span style="font-size: 48px;">⚠️</span></div>';
            ...
            $html .= '<td style="padding: 12px 16px; background-color: #ffffff; border-bottom: 1px solid #e9ecef; color: #333; font-size: 14px; word-break: break-word;">' . htmlspecialchars($error['type'] ?? 'Unknown') . '</td></tr>';
            ...
            $html .= '<td style="padding: 12px 16px; background-color: #ffffff; border-bottom: 1px solid #e9ecef; color: #d32f2f; font-size: 14px; word-break: break-word;"><strong>' . htmlspecialchars($error['message'] ?? 'No message') . '</strong></td></tr>';
            ...
            $html .= '<td style="padding: 12px 16px; background-color: #ffffff; border-bottom: 1px solid #e9ecef; color: #333; font-size: 13px; font-family: \'Courier New\', monospace; word-break: break-all;">' . htmlspecialchars($error['file'] ?? 'Unknown') . '</td></tr>';
            ...
            $html .= '<tr><td style="padding: 12px 16px; background-color: #f8f9fa; border-bottom: 1px solid #e9ecef; color: #666; font-size: 14px;"><strong>URL:</strong></td>';
            $html .= '<td style="padding: 12px 16px; background-color: #ffffff; border-bottom: 1px solid #e9ecef; color: #2196f3; font-size: 13px; word-break: break-all;">' . htmlspecialchars($request['url'] ?? '') . '</td></tr>';
            $html .= '<tr><td style="padding: 12px 16px; background-color: #f8f9fa; color: #666; font-size: 14px;"><strong>Timestamp:</strong></td>';
            $html .= '<td style="padding: 12px 16px; background-color: #ffffff; color: #333; font-size: 14px;">' . htmlspecialchars($timestamp) . '</td></tr>';
            $html .= '</table>';
            ...
            $html .= '<pre style="margin: 0; font-family: \'Courier New\', monospace; font-size: 12px; line-height: 1.5; color: #333; white-space: pre-wrap; word-wrap: break-word;"> ' . $errorData['trace'] . ' </pre>';
            $html .= '</div>';
            $html .= '</div>';
            $html .= '</td></tr>';
        } else {
            ...
            // Client email body
            $html .= '<tr><td style="padding: 40px 30px;">';
            $html .= '<p style="color: #333; font-size: 16px; line-height: 1.6; margin: 0 0 20px 0;">This is an automated notification from ' . htmlspecialchars($storeName) . '.</p>';
            $html .= '<div style="background-color: #fff3cd; border-left: 4px solid #ff9800; padding: 16px 20px; margin-bottom: 20px; border-radius: 4px;">';
        ...
        $html .= '<p style="margin: 0 0 8px 0; color: #666; font-size: 12px;">Automated alert from <strong>' . htmlspecialchars($storeName) . '</strong></p>';
